	...
	for(i = 0; i < mu; i++){
		if (drand(1) <= individual_mutation_probability){ 
			if (variable_mutation_probability > 0){
				...
				if(result != 0)	log_to_file(log_file, __FILE__, __LINE__,"mutation failed!");
			}
		}
	}
...
int eval(individual *ind){
	...
	//Seccion del codigo de markowitz
	if(strcmp(problem, "MARKOWITZ") == 0){
		return (eval_MARKOWITZ(ind));
	}
...
int read_data(){
	...
	for (int i = 0; i < number_decision_variables; ++i){//Carga de la matriz de varianzas y covarianzas
		for (int j = 0; j < number_decision_variables; ++j)	{
			fscanf(fp,"%s", str);
			cov_mat[i][j]=atof(str);			
		}
...
int reserve_memory(){
	...
	exp_ret = (double *)malloc(number_decision_variables * sizeof(double));

	if(!exp_ret)	return -1;	   

	cov_mat = (double **)malloc(number_decision_variables * sizeof(double));

	if(!cov_mat) return -1;

	for (int i = 0; i < number_decision_variables; ++i){
		cov_mat[i] = (double *)malloc(number_decision_variables * sizeof(double));
		if(!cov_mat[i]) return -1;		
	}
	return 0;
}
